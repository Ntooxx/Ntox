...
export class MentalModel {
  ...
  extractFromConversation(text: string): void {
    ...
    for (const pattern of OPINION_PATTERNS) {
      ...
      if (match && match[1]) {
        ...
        if (s.length >= 5 && s.length <= 150 && !extracted.some((e) => e.includes(s.slice(0, 20)))) {
          extracted.push(s);
          this.recordEntry(s, "opinion");
        }
      }
    }
    ...
    for (const pattern of GENERALIZATION_PATTERNS) {
      ...
      if (match) {
        ...
        if (s.length >= 5 && s.length <= 150 && !extracted.some((e) => e.includes(s.slice(0, 20)))) {
          extracted.push(s);
          this.recordEntry(s, "generalization");
        }
      }
    }
    ...
    for (const pattern of COMPARISON_PATTERNS) {
      ...
      if (match) {
        ...
        const s = `${a} over ${b}`;
        if (s.length <= 150 && !extracted.some((e) => e.includes(s.slice(0, 20)))) {
          extracted.push(s);
          this.recordEntry(s, "comparison");
        }
      }
    }
  ...
  private recordEntry(statement: string, category: MentalModelCategory): void {
    ...
    if (existing) {
      const temp = { id: "", statement, category, firstSeen: 0, lastSeen: 0, mentionCount: 0, sourceContext: "", contradictions: [], status: "active" as MentalModelStatus };
      if (this.isContradicting(temp, existing)) {
        ...
        existing.contradictions.push(entry.id);
    ...
    for (const other of this.entries) {
      ...
      if (this.isContradicting(entry, other)) {
  ...
  private isContradicting(a: MentalModelEntry, b: MentalModelEntry): boolean {
    const aText = a.statement.toLowerCase();
    const bText = b.statement.toLowerCase();

    for (const [wordA, wordB] of CONTRADICTION_PAIRS) {
      const aHasA = aText.includes(wordA);
      const aHasB = aText.includes(wordB);
      const bHasA = bText.includes(wordA);
      const bHasB = bText.includes(wordB);
    ...
    const aHasNegation = this.hasNegation(aText);
    const bHasNegation = this.hasNegation(bText);
  ...
  private hasNegation(text: string): boolean {
    ...
    // Check for "not X" or "don't X" patterns
  ...
  getContradictions(): { entry: MentalModelEntry; contradictedBy: MentalModelEntry }[] {
    ...
    for (const entry of this.entries) {
      for (const cId of entry.contradictions) {
        ...
        if (other) {
          result.push({ entry, contradictedBy: other });
        }
      }
    }
    return result;
